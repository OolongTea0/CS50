...
def register():
    ...
    if request.method == "POST":
        ...
        # Missing form submission or pw and confirmation are mismatched
        if not request.form.get("username"):
        ...
        # Query db for last id, remembers user and redirects to main page
        session["user_id"] = db.execute("SELECT MAX(id) FROM users")
        return redirect("/")

    else:
        #If method is get, returns register form
